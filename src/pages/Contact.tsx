...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Check for missing or placeholder URL
    ...
    const payload = {
      ...
      siteSource: 'juniorlacrosseband.com',
      formType: 'Booking Inquiry'
    };

    try {
      console.log(`[Contact] Sending transmission to: ${webhookUrl}`);
    ...
    return (
      <div className="animate-fade-in text-center py-24 space-y-8">
        ...
        <p className="text-xl text-stone-400 uppercase tracking-[0.2em] font-light max-w-md mx-auto leading-relaxed">
          We've got your message loud and clear. Junior will be in touch shortly.
        </p>
        <button
          onClick={() => setSubmitted(false)}
          className="text-brand-gold font-header uppercase tracking-[0.3em] text-sm hover:text-white transition-colors border-b border-brand-gold/30 py-2"
        >
        ...
        <div className="space-y-8">
          <div>
            ...
            <p className="text-stone-400 text-lg leading-relaxed">
              Want the Junior Lacrosse Band at your venue, festival, or private event? Fill out the form and we'll get back to you. We bring the party — you bring the dance floor.
            </p>
          </div>
          <div>
            <h2 className="text-2xl font-header uppercase tracking-widest text-brand-gold mb-4">Contact Information</h2>
            <p className="text-stone-400 text-lg leading-relaxed">
              Email: booking@juniorlacrosseband.com<br />
              Phone: (504) 813-6374
            </p>
          </div>
          <div>
            ...
            <div className="flex space-x-6">
              <a href="https://www.facebook.com/JuniorLacrosseBand" target="_blank" rel="noopener noreferrer" className="text-stone-400 hover:text-brand-gold transition-colors uppercase tracking-widest font-header">Facebook</a>
              <a href="https://open.spotify.com/artist/4cZ6AdaOgsH9Fgvv3NArgL" target="_blank" rel="noopener noreferrer" className="text-stone-400 hover:text-brand-gold transition-colors uppercase tracking-widest font-header">Spotify</a>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white/5 p-8 rounded-3xl border border-white/5">
          <div>
            ...
            <input
              ...
              className="w-full bg-neutral-950 border border-white/10 rounded-xl px-4 py-3 text-stone-50 focus:outline-none focus:border-brand-gold transition-colors"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              className="w-full bg-neutral-950 border border-white/10 rounded-xl px-4 py-3 text-stone-50 focus:outline-none focus:border-brand-gold transition-colors"
              placeholder="Tell us about your event — venue, date, and any details..."
            ></textarea>
          </div>
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-brand-gold hover:bg-brand-gold/80 text-neutral-950 font-header uppercase tracking-[0.2em] py-4 rounded-xl transition-all duration-300 transform hover:scale-[1.02] disabled:opacity-50 flex items-center justify-center gap-3"
          >
            {isLoading ? (
              <div className="h-6 w-6 border-2 border-neutral-950 border-t-transparent rounded-full animate-spin"></div>
            ) : (
          ...
          {error && (
            <div className="mt-6 border border-red-500/30 bg-red-500/5 p-4 rounded-lg">
              ...
              <button
                ...
                className="text-[10px] text-white/40 uppercase tracking-widest underline hover:text-brand-gold"
              >
              ...
              {showDebug && (
                <div className="mt-4 p-4 bg-black/60 rounded border border-white/10 font-mono text-[10px] text-brand-gold overflow-x-auto">
                  ...
                  <pre className="whitespace-pre-wrap">{JSON.stringify({
                    ...formData,
                    submittedAt: 'GENERATED_ON_SUBMIT',
                    siteSource: 'juniorlacrosseband.com'
                  }, null, 2)}</pre>
                  <div className="mt-4 p-2 bg-red-500/10 border border-red-500/20 text-red-400">
                    <p className="font-bold">⚠️ WEBHOOK TEST MODE NOTICE:</p>
                    <p>You are using a 'webhook-test' URL. This will fail if the n8n editor is closed or not in "Listen" mode. Switch to a production webhook URL for live use.</p>
                  </div>
                </div>
              )}
            </div>
